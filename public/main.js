...
console.log('CSRF Token:', getCsrfToken()); // Debug log
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                if (xhr.responseText.includes('invalid csrf token')) {
                    console.error('Invalid CSRF token during signup');
                    sendLog('error', currentUserEmail, 'Invalid CSRF token detected during signup');
                }
    ...
    xhr.onload = function() {
        ...
        if (xhr.status === 200) {
            ...
            if (response.error.includes('invalid csrf token')) {
                console.error('Invalid CSRF token during login');
                sendLog('error', currentUserEmail, 'Invalid CSRF token detected during login');
            }
